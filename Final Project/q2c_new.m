...
%         temp = bit2int(reshape(dataInEn,[2,256]),2);
        ...
        %precoding
        Uvec = zeros(2,2,length(data_scs));
        Svec = zeros(2,2,length(data_scs));
        for idx = 1:length(data_scs)
            scidx = data_scs(idx);
            [U,S,V] = svd([H11(scidx),H12(scidx);
                        H21(scidx),H22(scidx)]);
            Uvec(:,:,scidx) = U;
            Svec(:,:,scidx) = S;
            pr = V*[s1(scidx);s2(scidx)];
            s1(scidx) = pr(1);
            s2(scidx) = pr(2);
        end
...
%         temp = (H'*H+2/snr*eye(2*N))^-1*H'*r;
%       
        s1_noisy = zeros(length(data_scs),1);
        s2_noisy = zeros(length(data_scs),1);
        for idx = 1:length(data_scs)
            scidx = data_scs(idx);
            U = Uvec(:,:,scidx); 
            S = Svec(:,:,scidx);
            pr = U*[x1(scidx);x2(scidx)]./diag(S);
            s1_noisy(idx) = pr(1,1);
            s2_noisy(idx) = pr(2,1);
        end
...
legend_str ="Spatial Mult. 2x2 MIMO OFDM: Delay Spread:"+string(tau_rms*1e6)+"us"+" Pilots"+string(Npilots);
