function [k_list, avg_ber,Ne,enabled_scs] =  get_bitloading_rayleigh2(snr_linear, target_ber)
N = length(snr_linear);
pow_list= ones(N,1);
noise_pow_list = 1./snr_linear;

ber_per_sc = get_ber_rayleigh2(snr_linear,6*ones(N,1));
k_list = 6*ones(N,1);
...
while(avg_ber> target_ber)
    ...
    ber_per_sc = get_ber_rayleigh2(snr_linear,k_list);
    avg_ber = N/Ne*sum(ber_per_sc.*k_list)/(sum(k_list)); 
end