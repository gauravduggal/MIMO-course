    ...
    for Lidx=1:length(Lvec)
        ...
        parfor snr_idx = 1:length(snrvec)
            ...
            for trials=1:Ntrials
                ...
                H = randraw('nakagami', m, Nr).*exp(1j*2*pi*rand(Nr,1));
...
%                 sum(abs(H-H_est).^2)
        ...
        legend_label1 = string(scheme(kidx))+" L:"+string(Lvec(Lidx))+" K:"+string(kvec(kidx)+" imperfect csi"+" m:"+string(m));
        semilogy(snrvec,ber_imperfectcsi,'-x','DisplayName',legend_label1,'MarkerIndices',1:3:41);
        grid on
        hold on
        legend_label2 = string(scheme(kidx))+" L:"+string(Lvec(Lidx))+" K:"+string(kvec(kidx)+" perfect csi"+" m:"+string(m));
