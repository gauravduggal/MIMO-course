...
%             while errorStats(2) < 20 && errorStats(3) < 2048*10
            for ctr = 1:3
                ...
                temp = reshape(tx,[N,12]);
                temp = transpose(temp);

                cp = temp(:,N-cp_n+1:N);
                temp2 = [cp,temp];
                %dont ask why is there a transpose, stupid matlab syntax
                tx_cp = reshape(transpose(temp2),[(N+cp_n)*12,1]);
                %create channel in time domain
                h = get_freq_selective_channel(tau_rms,fs,N);
                h = transpose(h);
                %this results in 12*(N+CP_N)+N-1 samples, the last N-1 samples is the
                %tail
                rx_cp = conv(tx_cp,h);
                ...
                temp = zeros(12,N);
                encoded_received_bits = zeros(12*N*k,1);
                for sidx = 1:12
                ...
                %             ber_snr = ber_snr +  1/(12*Ntrials)*bit_error_current_packet;
