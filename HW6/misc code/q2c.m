...
%         ber_per_sc(idx) = get_ber_rayleigh(snr_sc_db(idx),k_per_sc(idx));
...
%          k_per_sc = k_vs_sc_snr(1:N,snridx);
%          enabled_scs = enabled_scs_snr(:,snridx);
%         h = [1, 0.5, 0.35, 0.15, 0.05];
%         h = h/sqrt(sum(abs(h).^2));%transpose(get_freq_selective_channel(tau_rms,fs,N));
        h = get_freq_selective_channel(tau_rms,fs,L);
...
%         H = sqrt(N)*H;
        ...
        %initialise highest modulation order on all subcarriers
        k_per_sc = 6*ones(N,1);
%         theory_ber = transpose(get_ber_rayleigh(snr_db,6)*ones(N,1));
        theory_ber = transpose(get_ber_awgn2(10.^(snr_sc_db/10),6*ones(N,1)));
        ber_per_sc = theory_ber;
        enabled_scs = ones(N,1);
        %         throughput = target_ber*N*6*ofdm_symbols;
        avg_ber = sum(enabled_scs)/N * sum(k_per_sc(enabled_scs).*ber_per_sc(enabled_scs))/(sum(k_per_sc(enabled_scs)));
...
%             ber_per_sc(idx) = get_ber_awgn2(10.^(snr_sc_db(idx)/10),k_per_sc(idx));
            enabled_scs = (ber_per_sc>0);
            %             avg_ber = sum(k_per_sc(enabled_scs).*ber_per_sc(enabled_scs))/(mean(k_per_sc)*N);
            ber_per_enabled_sc = sum(k_per_sc(enabled_scs).*ber_per_sc(enabled_scs))/sum(k_per_sc(enabled_scs));
            avg_ber = ber_per_enabled_sc;
%             avg_ber = sum(enabled_scs)/N * sum(k_per_sc(enabled_scs).*ber_per_sc(enabled_scs))/(sum(k_per_sc(enabled_scs)));
        end
%         if isnan(avg_ber)
%             continue
%         end
%         if sum(enabled_scs) == 0
