    ...
    parfor snridx=1:length(snrvec)
        ...
        for trials=1:Ntrials
            ...
            for idx=1:Nt*k:Npilots*Nt*k-Nt*k
                ...
                for nridx = 1:Nr
                for ntidx=1:Nt
                    sym = qammod(sym_dec(ntidx),2^k,'UnitAveragePower',true);
                    rx(nridx) = add_noise(H(nridx,ntidx)*sym,snr,k);
                    H_est(nridx,ntidx) = H_est(nridx,ntidx) + 1/(Npilots) * (rx(nridx)*sym');
                end
                end
            end
